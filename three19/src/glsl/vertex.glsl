...
void main() {
    ...
    vNormal = normal;
    vec3 p = position;
    float noise = noised(4.*vec3(p.x,p.y + 0.2*cos(2.*PI*playHead),p.z + 0.2*sin(2.*PI*playHead))).x;
    noise = noise*(2. - noise);
    vNoise = noise;
    vec3 newposition = position + 0.1*noise*normalize(position);
